...
//
//	Author:			Huang Yi Chan

// title of these windows:

const char *WINDOWTITLE = { "Huang Yi Chan's Project 3" };
...
// draw the complete scene:glRotatef( 360.*Time, 0., 1., 0. );
...
Display( )
{
	...
	*/
	...
	//
	// the projection matrix is reset to define a scene whoseF
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Huang Yi Chan Project3" );
...
InitMenus( )
{
	...
	glutAddMenuEntry("Without distortion", NoDistortion);
	glutAddMenuEntry("With distortion", Distortion);
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			if (DistortOn) {
				p->s = (lng + M_PI) / (2. * M_PI) + sin(2. * (lat + M_PI / 2. + Count)) * 0.20;
				p->t = (lat + M_PI / 2.) / M_PI;
			}
			else {
				p->s = (lng + M_PI) / (2. * M_PI);
